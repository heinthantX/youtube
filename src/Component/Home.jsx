...
export default function Home() {
  ...
  const searchClickListener = (searchKeyword) => {
    const api_key = 'b0fa49a28350a881c883e5ebaf7cfccd';
    const url = `https://api.themoviedb.org/3/search/movie?api_key=${api_key}&language=en-US&query=${searchKeyword}&page=1&include_adult=false`;
    setVideo([]);
    axios
      .get(url)
      .then((res) => {
  ...
  const tabsClickListener = (currentTab) => {
    setVideo([]);
    const api_key = 'b0fa49a28350a881c883e5ebaf7cfccd';
    const url = `https://api.themoviedb.org/3/discover/movie?api_key=${api_key}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_genres=${currentTab}&with_watch_monetization_types=flatrate`;
    axios
      .get(url)
      .then((res) => {
        setVideo(res.data.results);
        ...
        <div className=" pl-8">
          <div className="flex gap-8 w-full">
            ...
            <div className="bg-[#121212] w-full relative">
              <Tabs tabsClickListener={tabsClickListener} tabs={genres} />
              <div className=" mt-20 w-full">
                {loading ? (
                  ...
                  <div>
                    ...
                    <div>
                      <VideoCards videos={searchResult} />
                    </div>
                  </div>
                ) : (
